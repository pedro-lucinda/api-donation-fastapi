...
def create_cause(
    cause: schemas.CreateCause, cause_repository: CauseRepository = Depends(get_cause_repository)
):
    try:
...
def update_cause(
    ...
    This route receives a cause_id in the path parameters and an UpdateCause schema in the request
    body, validates them, and uses the CauseRepository to update the cause in the database.
...
def get_cause(
    ...
    This route receives a cause_id in the path parameters and uses the CauseRepository to get the cause from the database.
...
@cause_routes.put("/activate/{user_id}/{cause_id}", response_model=schemas.CauseSchema)
def toogle_active_cause(
    ...
    This route receives a cause_id in the path parameters and uses the CauseRepository to activate or deactivate the cause in the database.
